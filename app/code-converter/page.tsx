...
const languages = [
  ...
  { name: 'C++', color: 'bg-blue-700', hover: 'hover:bg-blue-800', shadow: 'shadow-blue-300', text: 'text-blue-600' },
  ...
  { name: 'JavaScript', color: 'bg-yellow-400', hover: 'hover:bg-yellow-500', shadow: 'shadow-yellow-200', text: 'text-yellow-500' },
  ...
  { name: 'Swift', color: 'bg-orange-500', hover: 'hover:bg-orange-600', shadow: 'shadow-orange-200', text: 'text-orange-500' },
  { name: 'HTML', color: 'bg-orange-400', hover: 'hover:bg-orange-500', shadow: 'shadow-orange-200', text: 'text-orange-400' },
  { name: 'CSS', color: 'bg-blue-400', hover: 'hover:bg-blue-500', shadow: 'shadow-blue-200', text: 'text-blue-400' },
...
export default function CodeConverterPage() {
  ...
  const handleConvert = async () => {
    if (!inputCode.trim()) return toast.error("Enter code first!");
    setIsConverting(true);
    setOutputCode('');
    try {
      await new Promise(resolve => setTimeout(resolve, 1500));
      setOutputCode(`// Optimized and Converted to ${targetLang}\n\n${inputCode}`);
  ...
  const handleDownload = () => {
    ...
    link.download = `code-cd.${targetLang.toLowerCase().replace(' ', '')}`;
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-10 flex flex-col items-center font-sans">
      ...
      <div className={`w-full max-w-7xl transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        ...
        <div className="text-center mb-10 mt-8 h-32">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-6 mb-12">
          ...
          <div className="w-full flex-1 bg-[#0F172A] rounded-3xl shadow-2xl overflow-hidden border border-slate-800">
            ...
            <textarea value={inputCode} onChange={(e) => setInputCode(e.target.value)} placeholder="Paste code or upload file..." className="w-full h-[400px] bg-transparent text-emerald-400 font-mono p-6 outline-none resize-none" />
          </div>
        ...
        <div className="bg-white/70 backdrop-blur-2xl border border-white/40 rounded-[3rem] p-10 shadow-[0_30px_60px_rgba(0,0,0,0.08)]">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-6 max-w-3xl mx-auto">
            <button onClick={handleConvert} disabled={isConverting || !inputCode} className={`group flex-1 w-full text-white px-10 py-6 rounded-3xl font-black text-2xl flex items-center justify-center gap-4 shadow-2xl transition-all duration-500 disabled:opacity-50 ${activeFormat.color} ${activeFormat.hover} ${activeFormat.shadow}`}>
